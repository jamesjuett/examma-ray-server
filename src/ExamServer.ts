...
import { db_getCollaborativeGradingServersByExamInstance, db_upsertCollaborativeGradingServerForExamInstanceAndQuestion } from "./db/db_collaborative_grading";
...
export class ExamInstanceServer {
  ...
  public async createNewCollaborativeGradingServer<CGKind extends CollaborativeGraderKind = CollaborativeGraderKind>(
    question_id: string,
    grader_kind: CGKind,
  ) {
    const new_server_config = await CollaborativeGradingServer.createNewServer(question_id, grader_kind);
    
    await db_upsertCollaborativeGradingServerForExamInstanceAndQuestion(this.exam_instance_uuid, question_id, new_server_config.grading_server_pk);

    await this.loadGradingServer(new_server_config.grading_server_pk);
    
    return new_server_config;
  }

  private async loadGradingServer(grading_server_pk: number) {
    const grading_server = await CollaborativeGradingServer.getOrLoadInstance(grading_server_pk);
    asMutable(this.collaborative_grading_servers_by_question_id).set(
      grading_server.question_id,
      grading_server
    );
  }
