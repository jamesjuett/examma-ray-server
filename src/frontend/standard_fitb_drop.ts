...
async function main() {
  ...
  const GRADING_SERVER_PK = qs["grading_server_pk"];
  assert(typeof EXAM_ID === "string");
  assert(typeof GRADING_SERVER_PK === "string");
  const app = await FITBDropGradingApp.create(EXAM_ID, parseInt(GRADING_SERVER_PK));
