...
exams_router
  ...
  .get(createRoute({
    ...
    handler: async (req, res: Response<readonly CollaborativeGradingServerConfig[]>) => {
      const exam_inst = EXAMMA_RAY_GRADING_SERVER
        .getExamServer(req.params["exam_id"])
        ?.getExamInstanceByUuid(req.params["exam_instance_uuid"]);
      ...
      return res.status(200).json(exam_inst.collaborative_grading_servers_by_question_id.values().map(server => server.getConfig()).toArray());
    }
  }));
...
exams_router
  ...
  .put(createRoute({
    ...
    handler: async (req, res) => {
      ...
      const existing_config = exam_inst.collaborative_grading_servers_by_question_id.get(req.params["question_id"]);
      if (existing_config) { return existing_config; }

      return res.status(200).json(exam_inst.createNewCollaborativeGradingServer(req.params["question_id"], "standard_fitb_drop"));
    }
  }))
