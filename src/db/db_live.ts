...
export async function db_createExamInstance(
  ...
  return (await query("exam_instances").insert({
...
export async function db_getExamInstancesByExamId(exam_id: string) {
  return await query("exam_instances").where({exam_id: exam_id}).select("*");
}

export async function db_getExamInstanceByUuid(exam_instance_uuid: string) {
  return await query("exam_instances").where({exam_instance_uuid: exam_instance_uuid}).select("*").first();
}
...
async function db_helper_getStudentExamInfo(orig_assn: DB_Live_Exam_Assignments) {
  const exam_instance = await db_getExamInstanceByUuid(orig_assn.exam_instance_uuid);
...
export async function db_getLiveExamAssignmentsByUniqname(uniqname: string) {
  return await query("live_exam_assignments")
    .join('exam_instances', 'exam_instances.exam_instance_uuid', '=', 'live_exam_assignments.exam_instance_uuid')
...
export async function db_getLiveExamAssignmentAndSubmission(exam_instance_uuid: string) {
  return await query("live_submissions")
    .join('live_exam_assignments', 'live_exam_assignments.exam_uuid', '=', 'live_submissions.exam_uuid')
    .where({exam_instance_uuid: exam_instance_uuid})
    .select('*');
