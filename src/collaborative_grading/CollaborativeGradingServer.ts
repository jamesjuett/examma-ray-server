...
import { db_getCollaborativeGradingServerConfig, db_getExamInstancesForGradingServer, db_getQuestionSubmissionsForGradingServer, db_updateCollaborativeGradingServerEpoch } from "../db/db_collaborative_grading";
...
export class CollaborativeGradingServer<CGKind extends CollaborativeGraderKind = CollaborativeGraderKind> {
  ...
  private async recordTransitions() {
    ...
    this.transitionRecorderJob = this.recordTransitions();
    ...
    // Note that the final check of the condition on the queue length in
    // the while loop above will happen and then synchronously we delete
    // the job promise, so it is guaranteed that the promise will never be present
    // unless there is an ongoing job to record transitions to the database.
